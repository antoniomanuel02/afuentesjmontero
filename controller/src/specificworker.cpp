...
/*
 ...
 cada componente va en un terminal
 
 el objetivo es que ahora vaya a sitios marcados en la pared o en las cajas utilizando la camara 
 
 le diremos al robot que las recorra en orden. 
 para aprovechar lo que hemos hecho en controller 
 para visitar las cuatro marcas tenemos que montar otra maquina de estados pero no en controller. 
 creamos un componente nuevo(supervisor) con cdsl ... s
 en el crearremos la maquina de estados nueva. Su primer estado es buscar la primera marca, girara hasta encontrar la primera, ahora ya se que 
 esta a la vista, tenemos que ir hacia el, y esperar a que llegue
 
 ahora me falta saber cuando ha llegado: 
 
 ese nuevo componente va a estar conectado a controller. 
 
 cada vez que encuentra un nuevo 
...
void SpecificWorker::buginit ( const TLaserData& ldata,const TBaseState& bState )
{
	...
	try
	{
		differentialrobot_proxy->setSpeedBase(40, 0.5);
	}
	catch ( const Ice::Exception &ex ) {  std::cout << ex << std::endl; }
	}
